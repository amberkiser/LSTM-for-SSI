...
def get_test_predictions(model, device, data_loader, sig_func):
    y_true = []
    y_prob = []
    pt_keys = []

    for inputs, labels, ids in data_loader.loader:
        inputs, labels = inputs.to(device), labels.to(device)
        output = model(inputs)
        output = sig_func(output)
        y_prob.extend(output.cpu().data.numpy())
    ...
    y_true = np.array(y_true)
    y_prob = np.array(y_prob)
    pt_keys = np.array(pt_keys)

    return y_true, y_prob, pt_keys
